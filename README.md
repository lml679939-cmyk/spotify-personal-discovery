...
3. **情境合成**（`app.py`）：自動偵測（IP → 城市 → 天氣 + **當地時間**）+ 文字描述
   + 圖片（Gemini Vision）+ 個人特質（MBTI/血型/星座）+ 心情雙軸 + 語言/曲風 + 投射問題
4. **LLM 推薦**（`recommend.py`）：Gemini 2.5 Flash 產生 JSON 候選（超額 1.6 倍），
   discovery / familiar 雙通道 prompt，並請 LLM 自評每首的知名度 `fame` 1-5
...
- **投射問題**：15 題隨機輪換（「你手機現在的桌布是什麼？」這類，用回答推測當下狀態）
...
- **讓 LLM 只提名歌手、曲目改由 Spotify 提供**（`artist_albums` + `album_tracks`），
  可根治「歌手真實存在但歌名是編的」這類幻覺——代價是每位歌手約 3 個請求，
  要先解決共用 Client ID 的速率限制
- 推薦結果評分回饋機制（目前沒有任何學習訊號）
